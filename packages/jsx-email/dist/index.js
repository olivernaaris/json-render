...
function renderElement(elementKey, spec, registry, stateModel, repeatItem, repeatIndex, repeatBasePath) {
  ...
  if (element.repeat) {
    const items = (0, import_core7.getByPath)(stateModel, element.repeat.statePath) ?? [];
    const repeat = element.repeat;
    const fragments = items.map((item, index) => {
      ...
      if (element.visible !== void 0 && !(0, import_core7.evaluateVisibility)(element.visible, itemCtx)) {
        return null;
      }
      const rawProps2 = element.props ?? {};
      const resolvedProps2 = (0, import_core7.resolveElementProps)(rawProps2, itemCtx);
      ...
      return /* @__PURE__ */ (0, import_jsx_runtime8.jsx)(Component2, { element: resolvedElement2, emit: noopEmit, children: children2 }, key);
    });
  ...
  const rawProps = element.props ?? {};
  const resolvedProps = (0, import_core7.resolveElementProps)(rawProps, ctx);
